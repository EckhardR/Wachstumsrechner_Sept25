...
const RoutineTasksView = () => {
  ...
  const loadTasks = () => {
    globalFetchDataV2(tasks, setTasks, () => API.TasksV2.getRoutineTasks(), setIsLoading, setTasksSecondData, setError, true);
  }
  ...
  const onSaveHandler = async(action, formData) => {
    ...
    switch(action) {
      case 'create':        
        response = await API.TasksV2.createRoutineTask(formData).catch( (error) => {          
          setError(() => {
              setIsLoading(false);
              return error
          });
      ...
      case 'edit':
        response = await API.TasksV2.updateRoutineTask(formData.ID, formData).catch( (error) => {
          setError(() => {
              setIsLoading(false);
              return error
          });
  ...
  const onDeleteHandler = (data) => {
    DialogConfirmation('DELETE', `Are you sure you want to delete this task: ${data?.Title}?`,
      async () => {
        const response = await API.TasksV2.deleteRoutineTask(data.ID).catch( (error) => {
          setError(() => {
              setIsLoading(false);
              return error
          });
  ...
  return (
    <Grid container spacing={2} sx={{p:2}}>  
      <RoutineDetailsModal details={details} onSave={onSaveHandler} onClose={ () => setDetails(null)} />
      <Grid item xs={12}>
