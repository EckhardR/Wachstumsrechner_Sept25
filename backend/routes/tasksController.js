...
router.post("/UpdateNewDailyTask", authenticateToken, async (req, res) => {
    ...
    console.log("All daily tasks inserted correctly", req.body);
    try {
        const taskDate = formatDate(taskData.TaskDate);
        const updateQuery = `
        ...
        const updateResults = await Promise.resolve(updateTaskPromise);
