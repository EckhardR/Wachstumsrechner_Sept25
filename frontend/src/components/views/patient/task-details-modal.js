...
export const TaskDetailsModal = (props) => {
  ...
  const isFormValid = () => {
    ...
    setDeadlineError(formData?.Deadline?.length <= 1);

    const err = [formData.Title, formData.Description, formData.Type, formData.Deadline]
        ...
        <Grid>
          ...
          <Grid item xs={6}>
            <TextField 
              ...
              error={DeadlineError}
              helperText={
                DeadlineError? 'Please select a valid date' : ''
              }
            />
          </Grid>
        </Grid>
